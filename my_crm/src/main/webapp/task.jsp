...
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/skin/css/base.css">
    ...
    <script type="application/javascript">
        $(function () {
            $.post("<%=path%>/Task/getAll",function (data) {
                $(data).each(function (index,item) {

                    var date01 = new Date(item.starttime)
                    var newDate01 = date01.getFullYear() +"-"+ (date01.getMonth()+1) +"-"+ date01.getDate()
                    var date02 = new Date(item.endtime)
                    var newDate02 = date02.getFullYear() +"-"+ (date02.getMonth()+1) +"-"+ date02.getDate()

                    var opt = "<tr align='center' bgcolor=\"#FFFFFF\" onMouseMove=\"javascript:this.bgColor='#FCFDEE';\" onMouseOut=\"javascript:this.bgColor='#FFFFFF';\" height=\"39\" >\n" +
                        "\t<td><input name=\"id\" type=\"checkbox\" id=\"id\" value=\"101\" class=\"np\"></td>\n" +
                        "\t<td>"+item.id+"</td>\n" +
                        "\t<td>"+item.tasktitle+"</td>\n" +
                        "\t<td align=\"center\">"+item.employee.ename+"</td>\n" +
                        "\t<td align=\"center\">"+(item.status==1?"进行中":"已完成")+"</td>\n" +
                        "\t<td align=\"center\">"+item.level+"</td>\n" +
                        "\t<td>"+newDate01+"</td>\n" +
                        "\t<td>"+newDate02+"</td>\n" +
                        "\t<td><a >正在进行</a> | <a >已完成</a> | <a href=\"task-edit.jsp\">编辑</a> | <a href=\"task-look.jsp\">查看详情</a></td>\n" +
                        "</tr>"
                    $("#tr2").after(opt)
                })

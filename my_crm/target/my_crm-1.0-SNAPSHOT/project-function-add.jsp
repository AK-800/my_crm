	...
	<script type="application/javascript">
       $(function () {
		   $.post("<%=path%>/Project/getProByFun",function(data) {
				$(data).each(function (index,item) {
						var opt = "<option value="+item.pname+","+item.pid+">"+item.pname+"</option>"
						$("#pro").append(opt)
				})
		   })
	   })
	   ...
	   function addAna(obj) {
		   ...
		   $.post("<%=path%>/Analysis/getInfoById",{id:id},function(data) {
			   $("#anaOpt").text(data.title)
			   $("#anaOpt").val(data.title)
		   })

		   //选择模块下拉框自动填充信息
		   $.post("<%=path%>/Module/queryModByAna",{id:id},function(data) {
			   $(data).each(function (index,item) {
...
<form name="form2" action="${pageContext.request.contextPath}/Function/insertFun" method="post" id="form5">
...
<tr >
	...
	<td  align='left' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22">
		<select id="anaId" name="analysisname" >
			<option id="anaOpt">选择需求</option>
		</select>
	</td>
</tr>
<tr >
